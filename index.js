...
function findById(movies, id) {
  ...
  try {
    ...
    if (found === undefined) {
      throw 'The movie cannot be found.'
    }
    return found
...
function filterByGenre(movies, genre) {
  ...
  try {
    const filmTitleByGenre = movies.filter((movie) => {
      return movie.genre.includes(genre[0].toUpperCase() + genre.slice(1).toLowerCase())
    })
    if (filmTitleByGenre.length === 0) {
      throw 'No movies match the genre.'
    }
    return filmTitleByGenre
...
function getAllMoviesReleasedAtOrBeforeYear(movies, year) {
  ...
  try {
    const allMoviesReleasedAtOrBeforeYear = movies.filter((movie) => {
      const releasedYear = movie.released.split(' ')
      return releasedYear[2] <= year
    })
    if (allMoviesReleasedAtOrBeforeYear.length === 0) {
      throw 'No movies were released at or before the given year.'
    }
